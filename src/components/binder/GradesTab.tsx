...
function TaskState({ task }: { task: Task }) {
  return (
    <>
      {task.done && <span className="grade-glyph" title="Done">✓</span>}
      {task.submitted && <span className="grade-glyph" title="Submitted">▣</span>}
    </>
  )
}
...
export default function GradesTab({ cls, onOpenUpload }: {
  ...
  const remove = (row: GradeRow) => {
    if (window.confirm(`Delete the "${row.name}" row?`)) dispatch({ type: 'deleteGradeRow', id: row.id })
  }
    ...
    <div className="grade-wrap">
      <div className="grade-summary">
        <div className="grade-stat">
          <span className="gs-label">Current grade <InfoIcon text={CURRENT_INFO} /></span>
          <span className="gs-value">
            {summary.currentGrade == null ? '—' : `${fmtPct(summary.currentGrade)}%`}
          </span>
        </div>
        <div className="grade-stat">
          <span className="gs-label">Running total <InfoIcon text={RUNNING_INFO} /></span>
          <span className="gs-value">{fmtPct(summary.runningTotal)}<span className="gs-sub"> / 100</span></span>
        </div>
        <div className="grade-stat">
          <span className="gs-label">Marked so far</span>
          <span className="gs-value">{fmtPct(summary.scoredWeight)}%<span className="gs-sub"> of the course</span></span>
        </div>
      </div>

      {summary.over && (
        <div className="grade-warn">
          The weights you've typed already add up to {fmtPct(summary.explicitTotal)}% — more than the whole
          course{summary.autoCount > 0 ? ', so the blank rows are left with nothing' : ''}.
        </div>
      )}

      {rows.length === 0 ? (
        <div className="empty-hint">
          Add a row for each assessed component — midterm, lab reports, final — and this tab keeps
          your running grade for {cls.name}.
        </div>
      ) : (
        <div className="grade-table">
          <div className="grade-row grade-head">
            <span>Component</span>
            <span>Weight <InfoIcon text={AUTO_INFO} /></span>
            <span>Score</span>
            <span>Assignments</span>
            <span>Documents <InfoIcon text={DOCS_INFO} /></span>
            <span />
          </div>
          ...
          {summary.rows.map(({ row, weight, auto }) => {
            ...
            return (
              <div key={row.id} className="grade-row">
                <input className="grade-name" value={row.name} placeholder="Component name"
                  onChange={(e) => patch(row, { name: e.target.value })} />

                <span className="grade-cell">
                  <NumCell value={row.weightPct} placeholder="auto" title="Weight as a % of the course"
                    onCommit={(n) => patch(row, { weightPct: n })} />
                  {auto && <span className="grade-auto">{fmtPct(weight)}%</span>}
                </span>

                <span className="grade-cell">
                  <NumCell value={row.scorePct} placeholder="—" title="What you scored, as a %"
                ...
                <span className="grade-cell grade-binds">
                  {bound.map((t) => (
                    <span key={t.id} className="grade-chip" title={t.title}>
                      ...
                      <button type="button" className="attach-x" title="Unbind from this row"
                        onClick={() => toggleTask(row, t.id)}>×</button>
                    </span>
                  ))}
                  <PickerPopover label="+ Bind" placeholder="Search assignments…" minWidth={300}>
                    {(needle) => {
                        ...
                        return (
                          <div className="st-empty">
                            {candidates.length
                              ...
                              : 'No tasks in an assignments-flagged section of this class yet.'}
                          </div>
                        )
                      }
                      return shown.map((t) => {
                        ...
                        return (
                          <button key={t.id} type="button"
                            ...
                            <TaskState task={t} />
                            <span className="pk-opt-when">{when ? fmtFriendly(when) : 'undated'}</span>
                          </button>
                        )
                      })
                    }}
                ...
                <button type="button" className="grade-del" title="Delete this row"
