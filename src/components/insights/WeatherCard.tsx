...
import { intervalMeta, pct, type IntervalKey } from './insightsData'
...
export default function WeatherCard({ ival, range }: { ival: IntervalKey; range: HeatmapRange }) {
  ...
  const note =
    `How often each kind of weather was marked over ${heatmapRangeNoun(range)} — the span follows ` +
    `the interval at the top of the page ("${intervalMeta(ival).label}"). The daily log records one ` +
    'weather per day, so every marked day falls in exactly one row and the counts add up to the ' +
    'number of days marked. Days with no weather set are not counted at all.'

  return (
    <section className="ins2-card">
      <div className="ins2-card-head">
        <h2 className="ins2-h2">Weather</h2>
        <InfoIcon text={note} />
      </div>
      ...
      {summary.total === 0 ? (
        ...
        <>
          <div className="ins2-weather-sum">
            <b>{summary.total}</b> {summary.total === 1 ? 'day' : 'days'} marked of {days.length}
          </div>
